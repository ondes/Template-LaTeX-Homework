...
\usepackage{mathtools} % For mathematical typesetting
\usepackage{amssymb} % For mathematical symbols, etc.
\usepackage{graphicx} % Required to insert images
\usepackage{subfig} % Used for subfigures
\usepackage{lscape} % To make defined pages' orientation landscape
\usepackage{hyperref} %For URL
\usepackage[nottoc,numbib]{tocbibind}
\usepackage{multirow} % For tables with multiple rows merged
\usepackage{xcolor} % For custom colors to be used
\usepackage{listings} % Required for insertion of code
\usepackage[framed]{mcode} %  MATLAB code listing based on lstlisting 
